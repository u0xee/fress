...
pub trait Process_ {
    ...
    fn ingest_kv(&mut self, process_stack: &mut [Box<dyn Process>], k: &Value, v: &Value) -> Option<Value> {
        let (next, rest) = process_stack.split_last_mut().unwrap();
        next.ingest_kv(rest, k, v)
    }
    fn last_call(&mut self, process_stack: &mut [Box<dyn Process>]) -> Value {
        let (next, rest) = process_stack.split_last_mut().unwrap();
        next.last_call(rest)
    }
}

pub fn ingest2(process_stack: &mut [Box<dyn Process>], v: &Value) -> Option<Value> {
    let (top, rest) = process_stack.split_last_mut().unwrap();
    top.ingest(rest, v)
}

pub fn ingest2_kv(process_stack: &mut [Box<dyn Process>], k: &Value, v: &Value) -> Option<Value> {
    let (top, rest) = process_stack.split_last_mut().unwrap();
    top.ingest_kv(rest, k, v)
}

pub fn last_call2(process_stack: &mut [Box<dyn Process>]) -> Value {
    let (top, rest) = process_stack.split_last_mut().unwrap();
    top.last_call(rest)
}
...
impl<F: Fn(&Value) -> Value, Next: Process> Process for Map<F, Next> {
    fn ingest(&mut self, process_stack: &mut [Box<dyn Process>], v: &Value) -> Option<Value> {
